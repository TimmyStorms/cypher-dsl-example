 ...
 */
public final class CypherDslTest {
    ...
    public void executeCount() {
        ...
        final String countStatement = start(allNodes("n")).returns(as(count(identifier("n")), "nodeCount")).toString();
    ...
    public void traverse() {
        ...
        final ExecutionResult result = this.executionEngine.execute(start(nodesById("u", userId))
                .match(node("u").both(Relationships.LIKES).node("db").both(Relationships.USES).node("l"))
                .returns(as(identifier("u").property("name"), "userName"),
                        as(identifier("db").property("name"), "dbName"),
                        as(identifier("l").property("name"), "languageName")).toString());
